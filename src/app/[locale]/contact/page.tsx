          ...
          <form className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:border-[#007AFF] outline-none transition-all placeholder:text-white/10" placeholder={t('form.namePlaceholder')} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] uppercase font-bold tracking-widest text-white/40 px-2">{t('form.email')}</label>
                <input type="email" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:border-[#007AFF] outline-none transition-all placeholder:text-white/10" placeholder={t('form.emailPlaceholder')} />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-white/40 px-2">{t('form.message')}</label>
              <textarea rows={4} className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 focus:border-[#007AFF] outline-none transition-all placeholder:text-white/10 resize-none" placeholder={t('form.messagePlaceholder')}></textarea>
            </div>
            <button className="w-full py-5 rounded-2xl bg-[#007AFF] text-white font-bold uppercase tracking-[0.2em] shadow-blue-glow hover:bg-[#0056b3] transition-all">{t('form.send')}</button>
          </form>
        </div>
