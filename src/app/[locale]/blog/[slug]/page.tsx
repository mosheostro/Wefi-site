...
const SUPPORTED_LOCALES = ['en', 'ru', 'de', 'he'];
  ...
  return (
    <article className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="w-full h-[40vh] md:h-[60vh] relative">
        <img src={resolvedPost.image} alt={resolvedPost.title[safeLocale] ?? resolvedPost.title['en']} className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
          <div className="container mx-auto max-w-4xl">
            ...
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold font-outfit tracking-tighter text-white mb-6 leading-[1.1]">
              {resolvedPost.title[safeLocale] ?? resolvedPost.title['en']}
            </h1>
            <div className="flex items-center gap-6 text-sm text-white/50 font-bold uppercase tracking-widest">
              ...
              <span className="text-[#8A2BE2]">{resolvedPost.slug}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-4xl px-4 py-16 md:py-24">
        ...
        <div className="mb-12 p-6 glass-panel border border-white/5 bg-white/5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
           ...
           <a href={resolvedPost.source.startsWith('https') ? resolvedPost.source : '#'} target="_blank" rel="noopener noreferrer" className="text-xs font-bold uppercase tracking-[0.2em] text-[#007AFF] hover:text-white transition-colors flex items-center gap-2">
              View Source ↗
           </a>
        </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none prose-p:text-white/60 prose-headings:font-outfit">
           {formatContent(resolvedPost.content[safeLocale] ?? resolvedPost.content['en'])}
        </div>
        ...
        <div className="mt-24 pt-12 border-t border-white/10">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {blogPosts.filter(p => p.slug !== resolvedPost.slug).map(other => (
               <Link key={other.slug} href={`/blog/${other.slug}`} className="p-4 glass-panel hover:bg-white/5 transition-colors border border-white/5 rounded-xl group flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <div className="text-sm font-bold text-white/80 group-hover:text-[#007AFF] transition-colors line-clamp-2">{other.title[safeLocale] ?? other.title['en']}</div>
                 </div>
               </Link>
             ))}
           </div>
